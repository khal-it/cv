\cvsection{Languages}



\cvskill{German}{5}
% \divider

\cvskill{English}{4}
% \divider

\cvskill{Arabic}{3}


\cvsection{Education}

\cvevent{Computer Science (Informatik), B.Sc.}{TECHNICAL UNIVERSITY OF BERLIN}{Berlin, 2021}{}

\cvsection{Areas of Expertise}

% Programming Languages
{\printinfo{\faCode}{\large\textbf{Programming Languages}}
\quad
\begin{itemize}
\item  \textbf{Expert}: Dart
\item  \textbf{Proficient}: TypeScript, JavaScript, Python, Kotlin, Swift
\item  \textbf{Competent}: Java
\end{itemize}
}

\vspace{1em}

% Frameworks
{\printinfo{\faCogs}{\large\textbf{Frameworks}}
\quad
\begin{itemize}
\item  \textbf{Expert}: Flutter
\item  \textbf{Proficient}: ReactJS, Spring Boot, NestJs
\item  \textbf{Competent}: Vue.js, ARCore
\end{itemize}
}

\vspace{1em}

% Tools
{\printinfo{\faWrench}{\large\textbf{Tools}}
\quad
\begin{itemize}
\item  \textbf{Expert}: Firebase, Sentry, CI/CD
\item  \textbf{Proficient}: Docker, Sentry, Google Analytics, Contentful, Strapi, Terraform
\item  \textbf{Competent}: Maestro, Automated Testing, REST, LLM, WebRTC, Selenium, Scrapy, Beautiful Soup, Spacy, Salesforce, Shopware, GraphQL, NodeJS
\end{itemize}
}

\vspace{1em}

% AI Agents & Workflows
{\printinfo{\faLaptop}{\large\textbf{AI Agents \& Workflows}}
\quad
\begin{itemize}
\item  \textbf{Expert}: Claude Code, Spec-Driven Development
\end{itemize}
}

\vspace{1em}

% Databases
{\printinfo{\faDatabase}{\large\textbf{Databases}}
\quad
\begin{itemize}
\item  \textbf{Expert}: Firebase
\item  \textbf{Proficient}: PostgreSQL, MongoDB, S3, SQLlite
\item  \textbf{Competent}: MySQL, Redis, Elasticsearch
\end{itemize}
}




