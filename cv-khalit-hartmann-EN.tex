%%%%%%%%%%%%%%%%%
% This is an example CV created using altacv.cls (v1.1.3, 30 April 2017) written by
% LianTze Lim (liantze@gmail.com), based on the 
% Cv created by BusinessInsider at http://www.businessinsider.my/a-sample-resume-for-marissa-mayer-2016-7/?r=US&IR=T
% 
%% It may be distributed and/or modified under the
%% conditions of the LaTeX Project Public License, either version 1.3
%% of this license or (at your option) any later version.
%% The latest version of this license is in
%%    http://www.latex-project.org/lppl.txt
%% and version 1.3 or later is part of all distributions of LaTeX
%% version 2003/12/01 or later.
%%%%%%%%%%%%%%%%

%% If you want to use \orcid or the
%% academicons icons, add "academicons"
%% to the \documentclass options. 
%% Then compile with XeLaTeX or LuaLaTeX.
% \documentclass[10pt,a4paper,academicons]{altacv}

%% Use the "normalphoto" option if you want a normal photo instead of cropped to a circle
\documentclass[10pt,a4paper,normalphoto]{altacv}

% \documentclass[10pt,a4paper]{altacv}

%% AltaCV uses the fontawesome and academicon fonts
%% and packages. 
%% See texdoc.net/pkg/fontawecome and http://texdoc.net/pkg/academicons for full list of symbols.
%% When using the "academicons" option,
%% Compile with LuaLaTeX for best results. If you
%% want to use XeLaTeX, you may need to install
%% Academicons.ttf in your operating system's font %% folder.


% Change the page layout if you need to
\geometry{left=1cm,right=9cm,marginparwidth=6.8cm,marginparsep=1.2cm,top=1cm,bottom=1cm}

% Change the font if you want to.

% If using pdflatex:
\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage[default]{lato}
\usepackage[colorlinks=true,linkcolor=VividPurple,urlcolor=VividPurple,citecolor=black,filecolor=black]{hyperref}

% If using xelatex or lualatex:
% \setmainfont{Lato}

% Change the colours if you want to
\definecolor{VividPurple}{HTML}{1a7a7a}
\definecolor{SlateGrey}{HTML}{1a1a1a}
\definecolor{LightGrey}{HTML}{4a4a4a}
\colorlet{heading}{SlateGrey}
\colorlet{accent}{VividPurple}
\colorlet{emphasis}{SlateGrey}
\colorlet{body}{LightGrey}

% Change the bullets for itemize and rating marker
% for \cvskill if you want to
\renewcommand{\itemmarker}{{\small\textbullet}}
\renewcommand{\ratingmarker}{\scriptsize\faSquare}




%% sample.bib contains your publications
\addbibresource{sample.bib}


\begin{document}

\name{Khalit Hartmann}
\tagline{Freelance Fullstack Engineer — Mobile, Web \& AI}
\photo{3.1cm}{cv-photo.jpg}
% Cropped to square from https://en.wikipedia.org/wiki/Marissa_Mayer#/media/File:Marissa_Mayer_May_2014_(cropped).jpg, CC-BY 2.0







\personalinfo{%
  \printinfo{\faHome}{\href{https://khal.it}{khal.it}}
  \newline
  \printinfo{\faGithub}{\href{https://github.com/khal-it}{github.com/khal-it}}
  \newline
  \printinfo{\faLinkedinSquare}{\href{https://www.linkedin.com/in/khalit-hartmann}{linkedin.com/in/khalit-hartmann}}
  \newline
  \printinfo{\emailsymbol}{\href{mailto:development@khal.it}{development@khal.it}}
  \newline
  \printinfo{\phonesymbol}{\href{tel:+4917623368034}{+49 176 233 680 34}}
  \newline
  \location{Berlin, Germany}
}

%% Make the header extend all the way to the right, if you want.
\begin{fullwidth}
\makecvheader
\end{fullwidth}

\begin{fullwidth}
\cvsection{Profile}
I am a freelance fullstack engineer from Berlin with more than 8 years of experience building and shipping mobile and web products for companies such as MediaMarktSaturn, On, Brack and Purelei. I design, build and release end to end: Flutter apps, TypeScript backends (NestJS), modern web frontends (SvelteKit, React) and the CI/CD pipelines around them. My current focus is on AI/LLM-powered products and multi-channel e-commerce platforms.
\end{fullwidth}


\begin{fullwidth}
\noindent
\begin{minipage}[t]{0.47\linewidth}
\cvsection{Languages}
\textcolor{emphasis}{\textbf{German}} (Native)\quad
\textcolor{emphasis}{\textbf{English}} (C1)\quad
\textcolor{emphasis}{\textbf{Arabic}} (B2)
\end{minipage}%
\hfill
\begin{minipage}[t]{0.47\linewidth}
\cvsection{Education}
\textcolor{emphasis}{\textbf{Computer Science (Informatik), B.Sc.}}\\
\textcolor{accent}{\textbf{Technical University of Berlin}} \textperiodcentered{} 2021
\end{minipage}

\cvsection{Experience}

% Grip King
\cvevent{Freelance Fullstack Engineer}{Grip King}{01.05.2026 - Present}{}
\textbf{Task:} Architecting and building a multi-channel tire \& rim e-commerce platform for the German market.\\
\textbf{Achievements:}
\begin{itemize}
    \item Designed and built the platform end-to-end: NestJS API, SvelteKit storefront, and admin dashboard in a pnpm monorepo.
    \item Modeled EU tire-label compliance (EU 2020/740/EPREL), DOT tracking, and ledger-based inventory.
    \item Implemented 15-minute stock reservations preventing oversell on eBay, Kleinanzeigen, and Tyre24/ALZURA.
    \item Integrated Billbee OMS (orders, stock, invoicing, DATEV), lexoffice, Stripe, and PayPal.
    \item Set up security-scanned CI/CD (Gitleaks, Semgrep, dependency audit) with auto-deployed staging on Hetzner.
    \item Spec-driven development with 23 ADRs and a full testing pyramid (Vitest, Playwright).
\end{itemize}

\textbf{Technologies used:}\\[0.4em]
\cvtag{E-Commerce}
\cvtag{TypeScript}
\cvtag{NestJS}
\cvtag{SvelteKit}
\cvtag{Svelte 5}
\cvtag{Prisma}
\cvtag{PostgreSQL}
\cvtag{Redis}
\cvtag{BullMQ}
\cvtag{Tailwind CSS}
\cvtag{Stripe}
\cvtag{PayPal}
\cvtag{Billbee}
\cvtag{Docker}
\cvtag{Caddy}
\cvtag{GitHub Actions}
\cvtag{CI/CD}
\cvtag{Sentry}
\cvtag{TDD}

\vspace{1em}
\end{fullwidth}

\begin{fullwidth}
% Protofy GmbH & Co. KG - Purelei
\cvevent{Freelance Mobile App Developer}{\href{https://protofy.com/}{Protofy GmbH \& Co. KG}}{01.12.2025 - 01.06.2026}{}
\textbf{Task:} Developing the Purelei mobile e-commerce app with Shopify webview integration and native features.\\
\textbf{Achievements:}
\begin{itemize}
    \item Developing the Purelei e-commerce app for iOS and Android.
    \item Integrated Shopify-based webview for a seamless shopping experience.
    \item Implemented Firebase Analytics, Crashlytics, Cloud Messaging, and Remote Config.
    \item Set up consent management with Usercentrics SDK.
    \item Configured dev/prod flavor builds with Firebase environments.
    \item Implemented state management architecture using Riverpod.
\end{itemize}

\textbf{Technologies used:}\\[0.4em]
\cvtag{E-Commerce}
\cvtag{Flutter}
\cvtag{Dart}
\cvtag{Kotlin}
\cvtag{Swift}
\cvtag{Firebase}
\cvtag{Shopify}
\cvtag{Riverpod (Code Generation)}
\cvtag{AutoRoute}
\cvtag{Deep Linking}
\cvtag{Webview}
\cvtag{Remote Config}
\cvtag{Cloud Messaging}
\cvtag{Crashlytics}
\cvtag{CI/CD}
\cvtag{Secure Storage}
\cvtag{Freezed}
\cvtag{json\_serializable}
\cvtag{build\_runner}
\cvtag{mockito}
\cvtag{Clean Architecture}
\cvtag{SOLID}
\cvtag{iOS}
\cvtag{Android}

\vspace{2em}

% Protofy GmbH & Co. KG - Brack
\cvevent{Freelance Mobile App Developer}{\href{https://protofy.com/}{Protofy GmbH \& Co. KG}}{01.05.2025 - 01.06.2026}{}
\textbf{Task:} Developed and released a mobile e-commerce app for the Swiss online shop Brack.\\
\textbf{Achievements:}
\begin{itemize}
    \item Successfully developed and launched the Brack e-commerce app.
    \item Implemented Firebase Remote Config for flexible app configuration.
    \item Integrated Firebase Cloud Messaging for push notifications.
    \item Set up CI/CD pipelines using GitHub Actions.
\end{itemize}

\textbf{Technologies used:}\\[0.4em]
\cvtag{E-Commerce}
\cvtag{Flutter}
\cvtag{Dart}
\cvtag{Kotlin}
\cvtag{Swift}
\cvtag{Firebase}
\cvtag{Remote Config}
\cvtag{Cloud Messaging}
\cvtag{Riverpod (Code Generation)}
\cvtag{AutoRoute}
\cvtag{Deep Linking}
\cvtag{GitHub Actions}
\cvtag{CI/CD}
\cvtag{Secure Storage}
\cvtag{Freezed}
\cvtag{json\_serializable}
\cvtag{build\_runner}
\cvtag{mockito}
\cvtag{Clean Architecture}
\cvtag{SOLID}
\cvtag{iOS}
\cvtag{Android}

\vspace{2em}

% SnapNext GmbH
\cvevent{Freelance Team Lead - Mobile App Development}{\href{https://snapnext.de/}{SnapNext GmbH}}{01.08.2024 - 01.03.2025}{}
\textbf{Personnel responsibility:} 2 Developers\\
\textbf{Task:} Led the mobile app development team for the Medi app, focusing on feature delivery and codebase improvements.\\
\textbf{Achievements:}
\begin{itemize}
    \item Increased daily subscriptions significantly within 6 months.
    \item Boosted user engagement with OneSignal push notifications.
    \item Launched a new podcast feature for the Medi app.
    \item Implemented in-app messaging, notifications, and promotions.
    \item Integrated Maestro for automated testing.
    \item Enhanced codebase scalability and maintainability.
\end{itemize}

\textbf{Technologies used:}\\[0.4em]
\cvtag{Health \& Lifestyle}
\cvtag{Team Leadership}
\cvtag{Push Notifications}
\cvtag{One Signal}
\cvtag{In App Notifications}
\cvtag{Automated Testing}
\cvtag{Flutter}
\cvtag{Dart}
\cvtag{Kotlin}
\cvtag{Swift}
\cvtag{Docker}
\cvtag{Python}
\cvtag{graphql\_flutter}
\cvtag{Auth/Token Refresh}
\cvtag{Link Chains}
\cvtag{GCP}
\cvtag{Firebase}
\cvtag{Sentry}
\cvtag{Google Analytics}
\cvtag{CI/CD}
\cvtag{Secure Storage}
\cvtag{Freezed}
\cvtag{json\_serializable}
\cvtag{build\_runner}
\cvtag{mockito}
\cvtag{Clean Architecture}
\cvtag{SOLID}
\cvtag{WebRTC}
\cvtag{SRTP}
\cvtag{iOS}
\cvtag{Android}
\cvtag{PostgreSQL}

\vspace{2em}

% On AG
\cvevent{Freelance Mobile App Developer}{\href{https://www.on.com/en-de/}{On AG}}{01.03.2024 - 01.08.2024}{}
\textbf{Task:} Developed and integrated advanced AI features and analytics into the ON App.\\
\textbf{Achievements:}
\begin{itemize}
    \item Spearheaded the AI-Chatbot Initiative within the ON App.
    \item Assisted in the development of the RAG-Pipeline.
    \item Launched the US Pilot for the ON App.
    \item Helped develop and implement an extensive analytics concept.
    \item Implemented automated tests with Maestro.
    \item Implemented advanced filtering feature.
\end{itemize}

\textbf{Technologies used:}\\[0.4em]
\cvtag{E-Commerce}
\cvtag{Flutter}
\cvtag{Dart}
\cvtag{Kotlin}
\cvtag{Swift}
\cvtag{Firebase}
\cvtag{Sentry}
\cvtag{Google Analytics}
\cvtag{REST}
\cvtag{LLM}
\cvtag{Chatbot}
\cvtag{Automated Testing}
\cvtag{GoRouter}
\cvtag{Deep Linking}
\cvtag{Secure Storage}
\cvtag{Freezed}
\cvtag{json\_serializable}
\cvtag{build\_runner}
\cvtag{mocktail}
\cvtag{Golden Tests}
\cvtag{Widget Tests}
\cvtag{Patrol E2E}
\cvtag{Trunk Based Development}
\cvtag{Clean Architecture}
\cvtag{SOLID}
\cvtag{Docker}
\cvtag{Contentful}
\cvtag{iOS}
\cvtag{Android}
\cvtag{\href{https://www.on.com/en-de/explore/mobile-app}{On App}}

\newpage
% Roodie Health GmbH
\cvevent{Freelance Mobile App Developer}{\href{https://www.roodie-health.com/}{Roodie Health GmbH}}{01.10.2023 - 01.03.2024}{}
\textbf{Personnel responsibility:} 1 Developer\\
\textbf{Task:} Led the rewrite of the React Native app in Flutter and managed white label development.\\
\textbf{Achievements:}
\begin{itemize}
    \item Rewrote the existing React Native App in Flutter.
    \item Led White Label Development for Roodie Health App.
    \item Implemented CI/CD Pipelines for development, deployment and testing.
    \item Provided technical consulting on best practices.
\end{itemize}

\textbf{Technologies used:}\\[0.4em]
\cvtag{Health Tech}
\cvtag{Flutter}
\cvtag{Dart}
\cvtag{Kotlin}
\cvtag{Swift}
\cvtag{Google Analytics}
\cvtag{Firebase Cloud Messaging / Push Notifications}
\cvtag{Sentry}
\cvtag{Strapi}
\cvtag{Riverpod}
\cvtag{Hive}
\cvtag{Freezed}
\cvtag{json\_serializable}
\cvtag{build\_runner}
\cvtag{mockito}
\cvtag{Monorepo Branching}
\cvtag{Fastlane}
\cvtag{REST}
\cvtag{Java}
\cvtag{Spring Boot}
\cvtag{CI/CD}
\cvtag{Clean Architecture}
\cvtag{SOLID}
\cvtag{iOS}
\cvtag{Android}
\cvtag{PostgreSQL}
\cvtag{\href{https://www.roodie-health.com/}{Roodie App}}

\vspace{2em}

% MediaMarktSaturn Technology GMBH
\cvevent{Flutter Engineer / Backend Engineer}{\href{https://careers.mediamarktsaturn.com/MMSTech/?locale=en_US}{MediaMarktSaturn Technology GMBH}}{15.09.2022 - 29.02.2024}{}
\textbf{Task:} Developed new features and optimized CI/CD pipelines for MediaMarkt and Saturn apps, and improved omnichannel communication.\\
\textbf{Achievements:}
\begin{itemize}
    \item Worked in a team of more than 30 individuals, including developers, UI/UX designers, product owners, and stakeholders.
    \item Implemented new features in the MediaMarkt and Saturn apps (Team Skittles).
    \item Optimized CI/CD pipelines and deployed apps to ensure seamless operations.
    \item Improved omnichannel communication for MediaMarkt and Saturn (Team Shiba Inu).
    \item Developed event-based push notification service.
    \item Implemented custom NFC plugin and forked payment plugin via Platform Channels (Swift/Kotlin).
    \item Delivered tech talks on Whitelabel App Development and Smart Push Notifications.
\end{itemize}

\textbf{Technologies used:}\\[0.4em]
\cvtag{E-Commerce}
\cvtag{Flutter}
\cvtag{Dart}
\cvtag{Kotlin}
\cvtag{Swift}
\cvtag{Firebase}
\cvtag{Sentry}
\cvtag{GCP}
\cvtag{Google Analytics}
\cvtag{graphql\_flutter}
\cvtag{Auth/Token Refresh}
\cvtag{Link Chains}
\cvtag{TypeScript}
\cvtag{Contentful}
\cvtag{Terraform}
\cvtag{NestJs}
\cvtag{NodeJS}
\cvtag{GoRouter}
\cvtag{Deep Linking}
\cvtag{Secure Storage}
\cvtag{Platform Channels}
\cvtag{Freezed}
\cvtag{json\_serializable}
\cvtag{build\_runner}
\cvtag{mockito}
\cvtag{Golden Tests}
\cvtag{Widget Tests}
\cvtag{Monorepo Branching}
\cvtag{Fastlane}
\cvtag{CI/CD}
\cvtag{Clean Architecture}
\cvtag{SOLID}
\cvtag{iOS}
\cvtag{Android}
\cvtag{Java}
\cvtag{\href{https://play.google.com/store/apps/details?id=com.media.markt&hl=en_US&pli=1}{MediaMarkt Germany}}
\cvtag{\href{https://play.google.com/store/apps/details?id=com.media.saturn&hl=en&gl=US}{Saturn}}
\cvtag{\href{https://play.google.com/store/apps/details?id=es.mediamarkt.app&hl=en_US}{MediaMarkt Spain}}
\cvtag{\href{https://play.google.com/store/apps/details?id=nl.com.media.markt&hl=nl&gl=US}{MediaMarkt Netherlands}}
\cvtag{\href{https://play.google.com/store/apps/details?id=com.media.markt&hl=en_US&pli=1}{MediaMarkt Italy}}

\vspace{2em}

% FRAMEGRABBER Medien GmbH
\cvevent{Freelance Mobile App Developer}{\href{https://www.framegrabber-medien.com/}{FRAMEGRABBER Medien GmbH}}{01.10.2021 - 01.07.2022}{}
\textbf{Task:} Developed two media guide apps and maintained close client communication.\\
\textbf{Achievements:}
\begin{itemize}
    \item Implemented two media guide Apps: DA Kunsthaus Kinder Spiel and DA Kunsthaus Geschichte.
    \item Delivered 2 high-quality apps with regular client updates.
\end{itemize}

\textbf{Technologies used:}\\[0.4em]
\cvtag{Education}
\cvtag{Flutter}
\cvtag{Dart}
\cvtag{Kotlin}
\cvtag{Swift}
\cvtag{Secure Storage}
\cvtag{Freezed}
\cvtag{json\_serializable}
\cvtag{build\_runner}
\cvtag{mockito}
\cvtag{Clean Architecture}
\cvtag{SOLID}
\cvtag{iOS}
\cvtag{Android}
\cvtag{\href{https://apps.apple.com/de/app/da-kunsthaus-kinderspiel/id6443640116}{DA, Kunsthaus Kinderspiel}}
\cvtag{\href{https://play.google.com/store/apps/details?id=com.framegrabber.gravenhorst_adults_app&gl=US}{DA, Kunsthaus Geschichte}}

\newpage
% SNAPNEXT GMBH (2020-2021)
\cvevent{Freelance Mobile App-/Web Developer}{\href{https://snapnext.de/}{SNAPNEXT GMBH}}{01.06.2020 - 01.09.2021}{}
\textbf{Personnel responsibility:} 1 Developer\\
\textbf{Task:} Developed and maintained mobile and web applications for various clients.\\
\textbf{Achievements:}
\begin{itemize}
    \item Developed and maintained apps such as Mein Orthomol, Orthomol - Virtuelle Apotheke, Mosaik, Tetrapak WEIHNACHTS-ORCHESTER, and EFFZEH 360.
    \item Maintained close communication with clients, providing regular updates and feedback.
\end{itemize}

\textbf{Technologies used:}\\[0.4em]
\cvtag{Health \& Lifestyle}
\cvtag{E-Commerce}
\cvtag{Flutter}
\cvtag{Dart}
\cvtag{Kotlin}
\cvtag{Swift}
\cvtag{ReactJS}
\cvtag{JavaScript}
\cvtag{CI/CD}
\cvtag{Firebase}
\cvtag{JAMStack}
\cvtag{Salesforce}
\cvtag{Shopware}
\cvtag{Strapi}
\cvtag{Vue.js}
\cvtag{GoRouter}
\cvtag{Deep Linking}
\cvtag{Secure Storage}
\cvtag{Freezed}
\cvtag{json\_serializable}
\cvtag{build\_runner}
\cvtag{mocktail}
\cvtag{Fastlane}
\cvtag{Clean Architecture}
\cvtag{SOLID}
\cvtag{iOS}
\cvtag{PostgreSQL}
\cvtag{Android}
\cvtag{Python}
\cvtag{\href{https://www.orthomol.com/de-de/service/app}{Mein Orthomol}}


\vspace{2em}

% TRIPPLE-A-CODE GMBH
\cvevent{Freelance App Developer}{TRIPPLE-A-CODE GMBH }{15.01.2020 - 01.06.2020}{}
\textbf{Task:} Led the development of the Holoswitch App and implemented custom WebRTC plugin.\\
\textbf{Achievements:}
\begin{itemize}
    \item Spearheaded the development of the Holoswitch App.
    \item Implemented custom WebRTC Flutter plugin via Platform Channels (Swift/Kotlin).
    \item Ensured seamless communication between the app and VR-Hardware.
    \item Set up CI/CD Pipelines for development, testing and deployment.
    \item Delivered high-quality applications tailored to client needs.
\end{itemize}

\textbf{Technologies used:}\\[0.4em]
\cvtag{Virtual Reality}
\cvtag{Flutter}
\cvtag{Dart}
\cvtag{Kotlin}
\cvtag{Swift}
\cvtag{WebRTC}
\cvtag{CI/CD}
\cvtag{Secure Storage}
\cvtag{Platform Channels}
\cvtag{Freezed}
\cvtag{json\_serializable}
\cvtag{build\_runner}
\cvtag{mockito}
\cvtag{Clean Architecture}
\cvtag{SOLID}
\cvtag{Vue.js}
\cvtag{iOS}
\cvtag{Android}
\cvtag{Java}
\cvtag{\href{https://bit.ly/3IHeeiF}{\textbf{Holoswitch}}}

\vspace{2em}

% PEQAS GMBH
\cvevent{Team Lead / App Developer}{PEQAS GMBH}{01.11.18 - 01.01.2020}{}
\textbf{Personnel responsibility:} 3 Junior Developers, 1 Designer\\
\textbf{Task:} Led the development of a cross-platform mobile application and managed CI/CD and AWS infrastructure.\\
\textbf{Achievements:}
\begin{itemize}
    \item Led the development of a cross-platform mobile application from scratch.
    \item Set up environments, Travis CI/CD pipelines, and AWS servers.
    \item Organized and led weekly sprint meetings.
    \item Gained practical experience in team management and agile development.
\end{itemize}

\textbf{Technologies used:}\\[0.4em]
\cvtag{Augmented Reality}
\cvtag{Flutter}
\cvtag{Dart}
\cvtag{Kotlin}
\cvtag{Swift}
\cvtag{CI/CD}
\cvtag{TypeScript}
\cvtag{Python}
\cvtag{Firebase}
\cvtag{Agile Project Management}
\cvtag{Secure Storage}
\cvtag{Freezed}
\cvtag{json\_serializable}
\cvtag{build\_runner}
\cvtag{mockito}
\cvtag{Clean Architecture}
\cvtag{SOLID}
\cvtag{Vue.js}
\cvtag{SRTP}
\cvtag{iOS}
\cvtag{Android}

\vspace{2em}

\newpage
\cvsection{Personal Projects}

% TessellAI
\cvevent{\href{https://tessellai.com}{TessellAI}}{Personal Project}{12.2024 - 03.2026}{}
\begin{itemize}
    \item Built a real-time photo-mosaic platform for live events: guests upload photos via QR code and watch them assemble into mosaic art live on screen.
    \item Engineered a computer-vision pipeline (OpenCV, NumPy, Jonker-Volgenant linear-assignment solver) with incremental per-tile updates streamed over WebSockets + Redis Pub/Sub.
    \item Migrated the platform from AWS serverless (SST/TypeScript) to a FastAPI/Python monolith with Celery workers and typed OpenAPI client generation.
\end{itemize}

\textbf{Technologies used:}\\[0.4em]
\cvtag{Python}
\cvtag{FastAPI}
\cvtag{Celery}
\cvtag{Redis}
\cvtag{PostgreSQL}
\cvtag{React}
\cvtag{TypeScript}
\cvtag{PixiJS}
\cvtag{OpenCV}
\cvtag{WebSockets}
\cvtag{Docker}
\cvtag{Vercel}

\vspace{1.5em}

% khal.it
\cvevent{\href{https://khal.it}{khal.it}}{Personal Project}{06.2026 - Present}{}
\begin{itemize}
    \item Designed and shipped a bilingual (DE/EN) portfolio and content platform with SvelteKit/Svelte 5, Tailwind v4, and Paraglide i18n.
    \item Built an SEO content engine: 21 keyword-researched articles with automated weekly republishing via GitHub Actions.
    \item Hardened the delivery pipeline: Gitleaks + Semgrep security scans, Lighthouse CI performance budgets, strict CSP; self-hosted on Hetzner with Docker/Caddy.
\end{itemize}

\textbf{Technologies used:}\\[0.4em]
\cvtag{SvelteKit}
\cvtag{Svelte 5}
\cvtag{TypeScript}
\cvtag{Tailwind CSS}
\cvtag{i18n}
\cvtag{SEO}
\cvtag{Vitest}
\cvtag{Playwright}
\cvtag{Lighthouse CI}
\cvtag{Docker}
\cvtag{Caddy}
\cvtag{Hetzner}

\vspace{1.5em}

% Norlin
\cvevent{\href{https://norlin.ai}{Norlin}}{Personal Project}{07.2026 - Present}{}
\begin{itemize}
    \item Building an AI-powered lead-outreach SaaS for marketing agencies: ad-funnel leads are engaged instantly by an AI conversation flow (Mirror, Qualify, Convert).
    \item Multi-tenant from day one: PostgreSQL row-level security via Prisma Client Extensions, queue-driven processing with BullMQ.
    \item Provider-agnostic LLM layer (Vercel AI SDK, Anthropic/OpenAI) with tool-calling; HubSpot OAuth and HMAC-verified webhooks; GDPR-by-design with EU data residency.
\end{itemize}

\textbf{Technologies used:}\\[0.4em]
\cvtag{AI/LLM}
\cvtag{NestJS}
\cvtag{SvelteKit}
\cvtag{TypeScript}
\cvtag{Prisma}
\cvtag{PostgreSQL RLS}
\cvtag{BullMQ}
\cvtag{Redis}
\cvtag{WebSockets}
\cvtag{HubSpot}
\cvtag{GDPR}

\vspace{1em}

\newpage
\cvsection{References}
\begin{quote}
``Wunderbare, professionelle Zusammenarbeit! K. hat den Grundstein für unsere gesamte mobile App Entwicklung gelegt. Sehr zu empfehlen!! `` - \textbf{Andre Gröschel, TrippleACode GmbH}\\
\end{quote}
\vspace{.5em}
\begin{quote}
``Professionelle, ergebnisorientierte, stets freundliche Zusammenarbeit. Ohne Einschränkung weiterzuempfehlen. Danke K.! `` - \textbf{Timo Mugele, Framegrabber GmbH}\\
\end{quote}

\vspace{.5em}

\begin{quote}
``K. hat uns mit klarer Kommunikation und vollem Einsatz überzeugt. Wir können die Zusammenarbeit mit ihm nur empfehlen ``- \textbf{Tim Schröder, MassiveMiniTeam GmbH}\\
\end{quote}

\vspace{.5em}
\begin{quote}
``Khalid has both, self-organization and autodidactic knowledge and abilities, thus he is stepping out, compared to regular flutter developers! `` - \textbf{Max Arbeiter, SnapNext GmbH}\\
\end{quote}

\vspace{.5em}    
\begin{quote}
``Superschnell ins Teamsetting eingefügt, immer hands-on Mentalität und Lösungsfokus, für einige Bereiche FE-Lead beim Re-Build der App übernommen" - \textbf{Nina Tamerl, Roodie Health GmbH}\\
\end{quote}

\vspace{.5em}
\begin{quote}
``K. was a very valued colleague in my department. He was very reliable and always happy to support others in the team.`` - \textbf{Andreas Partenhauser, MediaMarktSaturn Technology GmbH}\\
\end{quote}

\vspace{.5em}
\textcolor{red}{\small Please request permission before contacting references. Thank you.}
\end{fullwidth}

\begin{fullwidth}
\begin{center}
  \includegraphics[width=\linewidth]{company-logos-outline.png}
\end{center}
\end{fullwidth}


\end{document}
